...
class Node:
    def __init__(self, length):
        ...
        self.seen = 0
        self.different = 0
        self.children_counts = np.full(
            length + 1, Trie._minimum_count, dtype=np.int32)
...
class Trie:
    ...
    def update(self, buffer, offset, size):
        for context_size in range(size):
            self.root.update(buffer, offset + size - 1 -
...
class PPMEncoderModel(PPMBaseModel):
    ...
    def feed(self, next_tokens: List[Token]):
        ...
        self.context = (np.concatenate(
            [self.context, next_tokens], dtype=np.int32))[-(self.context_size + 1):]
...
class PPMModelDecoder(PPMBaseModel):
    ...
    def feed(self, next_tokens: List[Token]):
        ...
        if token == self._get_esc_token():
            raise Exception(
                "How can this be possible? This is surely a bug in code")
        self.context = (np.concatenate(
            [self.context, [token]]))[-(self.context_size + 1):]
        self.trie.update(self.context, 0, len(self.context))
        self.current_context = min(self.context_size, len(self.context))

    def get_frequencies(self) -> List[int]:
        while self.current_context >= 0:
            ok, freqs = self.trie.try_path(self.context, len(
                self.context) - self.current_context, self.current_context, False)
            if ok:
                return freqs
            self.current_context -= 1
        return self._get_uniform_frequencies()
