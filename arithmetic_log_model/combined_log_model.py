...
class ArithmeticLogEncoder:
    ...
    def encode_text(self, lines: List[str], input_file, output_file) -> None:
        self._write_to_stream(self.log_encoder.encode_int(
            len(lines)), self.auxillary_output_stream)
        for i, line in enumerate(lines):
            print("iteration", i)
            tokens = np.array(
                [ord(ch) for ch in line], dtype=np.int32)
...
#                 print("feeding", line[:start + length])
                ...
                self._write_to_stream([line[start]], self.integer_stream)
